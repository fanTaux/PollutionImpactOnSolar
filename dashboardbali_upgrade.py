...
def load_data():
    try:
        df = pd.read_csv("dataset_final_analisis_pv_BALI.csv")
        df['timestamp'] = pd.to_datetime(df['timestamp'])
        return df
    except FileNotFoundError:
        st.error("File 'dataset_final_analisis_pv_BALI.csv' tidak ditemukan. Pastikan file ada di folder yang sama.")
        return pd.DataFrame()
